...
- [x] **BUG-04:** 200-report cap — UI shows "most recent 200" notice when the limit is hit
- [x] **BUG-05:** Category drift — `ItemCategory` TS type widened to include `wallpaper`, `aluminum`, `hk`; CHECK constraint added to schema; seed updated with categories *(DB CHECK applied in 01-B; TS type widened in 01-D)*
...
- [x] **BUG-09:** Date filter timezone — `dateFrom` parsed as local `T00:00:00` to match `dateTo T23:59:59`
...
- [x] **BUG-13:** Floor grouping uses all-but-last-two digits as key (e.g. `"1001"` → floor `"10"`, not `"1"`)
- [x] **BUG-14:** Google Fonts `@import` replaced with `next/font`; `robots: noindex` added to layout metadata
...
|-------------|-------|--------|
| BUG-01 – BUG-14 | Phase 1 | Complete |
