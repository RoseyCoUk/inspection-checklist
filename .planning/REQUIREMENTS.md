...
- [ ] **BUG-01:** Double-submit race — `useRef` guard set synchronously before first `await` prevents two `INSERT`s from a fast double-tap
- [ ] **BUG-02:** Non-atomic submission — compensating delete of parent `reports` row on any mid-flow failure
- [ ] **BUG-03:** Unique index on `report_items(report_id, checklist_item_id)` prevents silent duplicate rows
- [ ] **BUG-04:** 200-report cap — UI shows "most recent 200" notice when the limit is hit
- [ ] **BUG-05:** Category drift — `ItemCategory` TS type widened to include `wallpaper`, `aluminum`, `hk`; CHECK constraint added to schema; seed updated with categories
- [ ] **BUG-06:** Orphan photos — storage objects deleted before the DB row on report delete
- [ ] **BUG-07:** N+1 writes replaced — single bulk `insert` for `report_items`, `Promise.all` for uploads within each item
- [ ] **BUG-08:** Photo uploads resized client-side (canvas, 1600px long edge, 0.7 JPEG quality, max 5 per item)
- [ ] **BUG-09:** Date filter timezone — `dateFrom` parsed as local `T00:00:00` to match `dateTo T23:59:59`
- [ ] **BUG-10:** Empty checklist items — `loaded` flag shows an empty-state message instead of an infinite spinner
- [ ] **BUG-11:** `localStorage` lang cast validated as `"en" | "ar"` before use
- [ ] **BUG-12:** Composite index on `reports(room_id, created_at desc)` for the hot-path "latest report per room" query
